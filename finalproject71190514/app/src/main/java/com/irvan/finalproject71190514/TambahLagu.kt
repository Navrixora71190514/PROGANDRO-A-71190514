...
import com.irvan.finalproject71190514.databinding.TambahmusikBinding
import java.net.URI
import java.text.SimpleDateFormat
import java.util.*

class TambahLagu : AppCompatActivity() {
    val cal = Calendar.getInstance()
    var firestore: FirebaseFirestore? = null
    lateinit var ImageUri : Uri
    lateinit var binding : TambahlaguBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = TambahlaguBinding.inflate(layoutInflater)
            ...
            if (judul!=null && penyanyi!=null && album!=null && genre!=null && kalendar!=null){
                if (acct!=null){
                    ...
                    if (firestore?.collection(personName.toString())
                        ...
                        firestore?.collection(personName.toString())?.document(DaftarLagu.judul)
                            ?.set(DaftarLagu)
